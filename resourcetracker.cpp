/*
ResourceTracker
A Windows command line utility for tracking resources (pdf, docx, doc, epub, chm, txt, zip etc.) in a  directory.
It works recursively finding all resources in the directory and subdirectories.

Building
Make an empty VisualStudio console application project and add the rt.cpp file and build the project.

For better workflow
After building this file into an exe, store the exe into a folder and add that folder path
...
If you want to include everytype of resources in the generated list
add a '*'
